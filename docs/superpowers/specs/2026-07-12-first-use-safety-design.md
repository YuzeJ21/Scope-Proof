...
Render the filtered matrix with Streamlit's static table component. Keep the existing per-criterion summary lines because they provide a compact textual status scan and preserve current AppTest-visible wording.
...
- Join the Markdown rows into one string. This would render a table, but user-authored criterion text containing pipe characters could still corrupt its structure.
- Use an interactive dataframe. This adds sorting and grid behavior that the first-use flow does not need and can make a small evidence summary harder to scan.
...
- Add Streamlit AppTest coverage proving the evidence matrix uses a rendered table rather than raw Markdown table syntax.
