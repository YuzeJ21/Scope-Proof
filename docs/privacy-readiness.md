...
| Fork credential exposure | Repository workflow uses the trusted base definition and never checks out or executes PR head code; planner returns no fork write plan | Continue payload fixture tests and external fork validation when a free, separately owned fork is available. |
