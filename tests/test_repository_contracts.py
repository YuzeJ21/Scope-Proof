...
def test_readme_separates_release_install_from_contributor_setup() -> None:
    ...
    assert (
        "https://github.com/YuzeJ21/Scope-Proof/releases/download/v0.1.21/"
...
def test_readme_documents_optional_release_checksum_verification() -> None:
    readme = Path("README.md").read_text(encoding="utf-8")
    wheel_name = "scopeproof-0.1.21-py3-none-any.whl"

    assert f"releases/download/v0.1.21/{wheel_name}" in readme
    assert f"releases/download/v0.1.21/{wheel_name}.sha256" in readme
...
def test_hatch_and_reviews_share_one_version_source() -> None:
    ...
    assert '__version__ = "0.1.22.dev0"' in version_source
