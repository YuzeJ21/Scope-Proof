...
def test_readme_documents_optional_release_checksum_verification() -> None:
    ...
    assert f"releases/download/v0.1.16/{wheel_name}" in readme
    assert f"releases/download/v0.1.16/{wheel_name}.sha256" in readme
...
def test_hatch_and_reviews_share_one_version_source() -> None:
    ...
    assert '__version__ = "0.1.17.dev0"' in version_source
